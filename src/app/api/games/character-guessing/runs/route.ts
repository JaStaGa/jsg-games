import {
  RANKED_CHARACTER_THEMES,
  validateRankedCharacterSubmission,
} from "@/games/character-guessing/logic/ranked-submission";
...
export async function POST(request: Request) {
  ...
  const validation = validateRankedCharacterSubmission(rawPayload);
  ...
  try {
    const { data, error } = await supabase
      .from("games")
      .select("id")
      .eq("slug", RANKED_CHARACTER_THEMES[validation.payload.themeId].gameSlug)
      .maybeSingle();

    if (error || !data || !Number.isSafeInteger(data.id) || data.id <= 0) {
      return errorResponse("service_unavailable", 503);
    }
